...
**Branch**: `001-shared-storage` | **Date**: 2026-01-13 | **Spec**: [spec.md](./spec.md)  
**Input**: Feature specification from `/specs/001-shared-storage/spec.md`  
**Status**: ✅ MVP Complete (Phase 1-3) + LXC Testing Support Added

**Note**: This implementation plan is generated by the `/speckit.plan` command.

## Summary

Implement shared storage architecture for Concourse CI charm to eliminate redundant binary downloads across multiple units. The feature enables units to share binaries from common storage, reducing disk overhead from N×binary size to ~1.15×.

**Implementation Completed**:
- File-based locking using Python's fcntl for download coordination
- Binary detection and version marker management
- Web/leader downloads once, workers wait and reuse binaries
- LXC testing support with `shared-storage=lxc` config option
- Marker file detection (`.lxc_shared_storage`) for LXC mode
- Removed Juju storage requirement (simplified deployment)

**Testing Approach**: LXC disk mounts with ID mapping (`shift=true`) for validation

## Technical Context

**Language/Version**: Python 3.12 (Ubuntu 24.04 base)  
**Primary Dependencies**: ops framework ≥2.0.0, fcntl (stdlib), pathlib (stdlib)  
**Storage**: LXC disk mounts with ID mapping for testing; no Juju storage required  
**Testing**: LXC shared storage with marker file detection, manual validation  
**Target Platform**: Ubuntu 24.04 LTS (Juju-managed LXD containers)  
...
- Phase 1-3 (34/34 tasks): Storage coordinator, web/leader download, worker waiting
- LXC testing support: Config option `shared-storage=lxc`, marker file `.lxc_shared_storage`
- Simplified storage: Removed `concourse-data` Juju storage, use `/var/lib/concourse` for LXC
- Testing: Manual LXC mount configuration with `readonly=false/true shift=true`
...
├── research.md                   # Phase 0: Juju storage patterns, locking ✅
├── data-model.md                 # Phase 1: Storage entities and state ✅
...
└── LXC_SHARED_STORAGE_TEST.md    # LXC testing procedure ✅ NEW
```
