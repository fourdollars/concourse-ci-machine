        ...
        try:
            ...
            for entry in self.base_path.iterdir():
                if entry.is_dir() and not entry.name.startswith("."):
                    folder = DiscoveredFolder(name=entry.name, path=str(entry))
                    result.folders.append(folder)
                    self.logger.debug(
                        f"Discovered folder: {folder.path} (writable={folder.is_writable})"
                    )
            ...
            self.logger.info(
                f"Folder discovery complete: {result.get_folder_count()} folders, {result.duration_ms}ms"
            )
    ...
    def generate_mount_args(self, folders: List[DiscoveredFolder]) -> List[str]:
        ...
        for folder in folders:
            ...
            if folder.is_writable:
                ...
                # Read-only mount
                mount_arg = (
                    f"type=bind,source={folder.path},target={folder.path},readonly"
                )
