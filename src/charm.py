...
try:
    ...
    logger.warning(
        "folder_mount_manager not available, folder discovery status disabled"
    )
...
class ConcourseCharm(CharmBase):
            ...
            if self._should_run_web():
                ...
                if storage_coordinator:
                    ...
                    try:
                        ...
                        logger.warning(
                            "Another unit is downloading binaries, deferring..."
                        )
                        self.unit.status = MaintenanceStatus(
                            "Another unit downloading, waiting..."
                        )
                        event.defer()
                        return
                    ...
                    logger.info(
                        "No shared storage configured, using local installation"
                    )
    ...
    def _on_upgrade_charm(self, event):
        ...
        try:
            ...
            logger.info(
                f"Charm upgrade triggered (mode: {self._get_deployment_mode()})"
            )
            ...
            if not (keys_dir / "tsa_host_key").exists():
                ...
                if verify_installation():
                    ...
                    logger.warning(
                        "Concourse binary missing, skipping key generation during upgrade"
                    )
        ...
        try:
            ...
            if desired_version:
                ...
                if (
                    ...
                    and self.config.get("shared-storage", "none") == "lxc"
                ):
                    from pathlib import Path
                    ...
                    if version_marker.exists():
                        ...
                        logger.info(
                            f"Found version {installed_version} in shared storage"
                        )
                ...
                if installed_version != desired_version:
                    ...
                    if use_coordinated_upgrade:
                        ...
                        if self.unit.is_leader() and self._should_run_web():
                            ...
                            # Check if upgrade is actually complete according to leader
                            # This handles cases where local detection fails but upgrade is done
                            is_complete = False
                            is_idle = False
                            try:
                                ...
                                if peer_relation:
                                    ...
                                    if app_data and "upgrade-state" in app_data:
                                        state = UpgradeState.from_relation_data(
                                            app_data
                                        )
                                        if (
                                            state.state == "complete"
                                            and state.target_version == desired_version
                                        ):
                                            is_complete = True
                                        elif state.state == "idle":
                                            is_idle = True
                            except Exception as e:
                                logger.warning(f"Failed to check upgrade state: {e}")

                            # If state is idle but versions mismatch, check if binaries are already there
                            # This handles the case where the complete signal was missed (T060)
                            if not is_complete and is_idle:
                                try:
                            ...
                            if is_complete:
                                ...
                                self.unit.status = WaitingStatus(
                                    f"Waiting for leader to install Concourse {desired_version}"
                                )
            ...
            if mode == "both":
                ...
                if self._should_run_worker():
                    ...
                    if not Path(CONCOURSE_BIN).exists():
                        logger.info(
                            "Concourse CI is not installed yet, skipping worker configuration"
                        )
                    ...
                    self.worker_helper.update_config(
                        tsa_host=tsa_host, keys_dir=keys_dir_path
                    )
    ...
    def _on_update_status(self, event):
        ...
        if self.config.get("shared-storage", "none") == "lxc" and not isinstance(
            ...
            if any(
                ...
                for keyword in ["storage", "mount", "waiting for leader", "install"]
            ):
                logger.info(
                    f"Unit in waiting state: {self.unit.status.message}, checking if can proceed..."
                )
                should_check_storage = True
        elif (
            isinstance(self.unit.status, BlockedStatus)
            and "not installed" in self.unit.status.message
        ):
            logger.info(
                "Unit blocked due to missing installation, checking if storage is available..."
            )
            should_check_storage = True
        ...
        if should_check_storage:
            ...
            if self.config.get("shared-storage", "none") == "lxc":
                ...
                if storage_path.exists() and marker_file.exists():
                    ...
                    if version_marker.exists():
                        ...
                        if self._should_run_worker():
                            ...
                            try:
                                ...
                                # generated by generate_keys() above, which creates a fresh
                                # (incorrect) tsa_host_key.pub in WORKER_KEYS_DIR.
                                if worker_only:
                                    import shutil
                    ...
                    logger.info(
                        "Shared storage detected but no binaries yet, triggering installation..."
                    )
                    ...
                    if self._should_run_web():
                        logger.info(
                            "Web unit: starting installation with shared storage"
                        )
                        self.unit.status = MaintenanceStatus(
                            f"Installing Concourse v{version}..."
                        )
                        try:
                            storage_coordinator = (
                                self.web_helper.initialize_shared_storage()
                            )
                            if storage_coordinator:
                                from concourse_installer import (
                                    download_and_install_concourse_with_storage,
                        ...
                        logger.info(
                            "Worker unit: starting installation with shared storage"
                        )
                        self.unit.status = WaitingStatus(
                            f"Waiting for leader to install Concourse CI {version}."
                        )
                        try:
                            storage_coordinator = (
                                self.worker_helper.initialize_shared_storage()
    ...
    def _on_database_changed(self, event):
        ...
        try:
            ...
            logger.info(
                f"Database data from library: {list(db_data.keys()) if db_data else 'None'}"
            )
    ...
    def _on_peer_relation_changed(self, event):
        ...
        if self._should_run_worker():
            ...
            if (
                not Path(CONCOURSE_BIN).exists()
                and not Path("/var/lib/concourse/bin/concourse").exists()
            ):
                logger.info(
                    "Concourse CI is not installed yet, skipping peer relation config"
                )
                # Set waiting status if we're waiting for shared storage
                if self.config.get("shared-storage", "none") != "none":
                    self.unit.status = WaitingStatus("Waiting for shared storage mount")
            ...
            if web_version and self.config.get("shared-storage", "none") == "none":
                ...
                if worker_version and worker_version != web_version:
                    logger.info(
                        f"Web version ({web_version}) differs from worker version ({worker_version}), upgrading..."
                    )
                    self.unit.status = MaintenanceStatus(
                        f"Auto-upgrading Concourse CI to {web_version}..."
                    )
                    ...
                    logger.info(
                        f"Worker auto-upgraded from {worker_version} to {web_version}"
                    )
    ...
    def _update_merged_config(self):
        ...
        if self.config.get("vault-url"):
            ...
            if self.config.get("vault-auth-backend"):
                web_config["CONCOURSE_VAULT_AUTH_BACKEND"] = self.config[
                    "vault-auth-backend"
                ]
            if self.config.get("vault-auth-backend-max-ttl"):
                web_config["CONCOURSE_VAULT_AUTH_BACKEND_MAX_TTL"] = self.config[
                    "vault-auth-backend-max-ttl"
                ]
            if self.config.get("vault-auth-param"):
                web_config["CONCOURSE_VAULT_AUTH_PARAM"] = self.config[
                    "vault-auth-param"
                ]
            ...
            if self.config.get("vault-client-cert"):
                web_config["CONCOURSE_VAULT_CLIENT_CERT"] = self.config[
                    "vault-client-cert"
                ]
            if self.config.get("vault-client-key"):
                web_config["CONCOURSE_VAULT_CLIENT_KEY"] = self.config[
                    "vault-client-key"
                ]
            if self.config.get("vault-client-token"):
                web_config["CONCOURSE_VAULT_CLIENT_TOKEN"] = self.config[
                    "vault-client-token"
                ]
            if self.config.get("vault-lookup-templates"):
                web_config["CONCOURSE_VAULT_LOOKUP_TEMPLATES"] = self.config[
            ...
            if self.config.get("vault-path-prefix"):
                web_config["CONCOURSE_VAULT_PATH_PREFIX"] = self.config[
                    "vault-path-prefix"
                ]
    ...
    def _update_status(self):
        ...
        try:
            ...
            if mode == "web":
                ...
                self.unit.status = ActiveStatus(
                    f"Worker ready{version_info}{gpu_status}{discovery_status}"
                )
            else:
                gpu_status = self.worker_helper.get_gpu_status_message()
                _, discovery_status = self._check_folder_discovery_status()
    ...
    def _orchestrate_coordinated_upgrade(self, event, version: str):
        ...
        try:
            ...
            logger.info(
                f"Step 1: Initiating upgrade, expecting {worker_count} workers"
            )  # T052
            ...
            logger.info(
                f"Coordinated upgrade to v{version} completed successfully"
            )  # T052
            self._update_status()
    ...
    def _handle_upgrade_signals(self, event):
        ...
        try:
            ...
            logger.info(
                f"Detected upgrade state: {upgrade_state.state} to v{upgrade_state.target_version}"
            )
            ...
            if not storage_coordinator:
                logger.warning(
                    "Storage coordinator not available for upgrade coordination"
                )
                return
    ...
    def _publish_version_to_peer_relation(self):
        ...
        try:
            ...
            logger.error(
                f"Failed to publish version to peer relation: {e}", exc_info=True
            )
    ...
    def _publish_version_to_tsa_relations(self):
        ...
        try:
            ...
            logger.error(
                f"Failed to publish version to TSA relations: {e}", exc_info=True
            )
        ...
        if self._should_run_web():
            ...
            try:
                ...
                if authorized_keys_path.exists():
                    existing_keys = set(
                        authorized_keys_path.read_text().strip().split("\n")
                    )
            ...
            try:
                ...
                if not tsa_host or not tsa_pub_key:
                    ...
                    self.unit.status = WaitingStatus(
                        "Waiting for TSA connection info..."
                    )
                    return
